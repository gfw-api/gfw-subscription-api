...
export const createGLADRADDGeostoreURLSubscriptionBody = (subscription: Record<string, any>, beginDate: Moment, endDate: Moment, bodyData: Record<string, any> = {}) => {
    ...
    return {
        ...
        wdpa_ha_sum: "10",
        downloadUrls: {
            csv: `${config.get('dataApi.url')}/dataset/gfw_integrated_alerts/latest/download/csv?sql=SELECT latitude, longitude, wur_radd_alerts__date, wur_radd_alerts__confidence FROM data WHERE wur_radd_alerts__date >= '${beginDate.format('YYYY-MM-DD')}' AND wur_radd_alerts__date <= '${endDate.format('YYYY-MM-DD')}'&geostore_origin=rw&geostore_id=423e5dfb0448e692f97b590c61f45f22`,
            json: `${config.get('dataApi.url')}/dataset/gfw_integrated_alerts/latest/download/json?sql=SELECT latitude, longitude, wur_radd_alerts__date, wur_radd_alerts__confidence FROM data WHERE wur_radd_alerts__date >= '${beginDate.format('YYYY-MM-DD')}' AND wur_radd_alerts__date <= '${endDate.format('YYYY-MM-DD')}'&geostore_origin=rw&geostore_id=423e5dfb0448e692f97b590c61f45f22`
        },
        glad_alert_type: "RADD deforestation alerts",
        layerSlug: "glad-radd",
...
export const mockGLADRADDISOQuery = () =>
...
export const mockGLADRADDAdm1Query = () =>
    nock(config.get('dataApi.url'))
...
export const mockGLADRADDAdm2Query = () =>
    nock(config.get('dataApi.url'))
...
export const mockGLADRADDWDPAQuery = () =>
    nock(config.get('dataApi.url'))
...
export const mockGLADRADDGeostoreQuery = (times: number = 1) =>
    nock(config.get('dataApi.url'))
