...
export const createGLADS2GeostoreURLSubscriptionBody = (subscription: Record<string, any>, beginDate: Moment, endDate: Moment, bodyData: Record<string, any> = {}) => {
            ...
            datasets: [
                {
                    ...
                    layers: [
                        'cb184858-4501-4e18-bdf4-8fc4150b9f6e'
                    ],
                    timelineParams: {
    ...
    return {
        ...
        wdpa_ha_sum: "10",
        downloadUrls: {
            csv: `${config.get('dataApi.url')}/dataset/gfw_integrated_alerts/latest/download/csv?sql=SELECT latitude, longitude, umd_glad_sentinel2_alerts__date, umd_glad_sentinel2_alerts__confidence FROM data WHERE umd_glad_sentinel2_alerts__date >= ${beginDate.format('YYYY-MM-DD')} AND umd_glad_sentinel2_alerts__date <= ${endDate.format('YYYY-MM-DD')}&geostore_origin=rw&geostore_id=423e5dfb0448e692f97b590c61f45f22`,
            json: `${config.get('dataApi.url')}/dataset/gfw_integrated_alerts/latest/download/json?sql=SELECT latitude, longitude, umd_glad_sentinel2_alerts__date, umd_glad_sentinel2_alerts__confidence FROM data WHERE umd_glad_sentinel2_alerts__date >= ${beginDate.format('YYYY-MM-DD')} AND umd_glad_sentinel2_alerts__date <= ${endDate.format('YYYY-MM-DD')}&geostore_origin=rw&geostore_id=423e5dfb0448e692f97b590c61f45f22`
        },
        glad_alert_type: "GLAD-S2 deforestation alerts",
        layerSlug: "glad-s2",
...
export const mockGLADS2ISOQuery = () =>
...
export const mockGLADS2Adm1Query = () =>
    nock(config.get('dataApi.url'))
...
export const mockGLADS2Adm2Query = () =>
    nock(config.get('dataApi.url'))
...
export const mockGLADS2WDPAQuery = () =>
    nock(config.get('dataApi.url'))
