...
export const createGLADAllGeostoreURLSubscriptionBody = (subscription: Record<string, any>, beginDate: Moment, endDate: Moment, bodyData: Record<string, any> = {}) => {
    ...
    return {
        ...
        wdpa_ha_sum: "10",
        downloadUrls: {
            csv: `${config.get('dataApi.url')}/dataset/gfw_integrated_dist_alerts/latest/download/csv?sql=SELECT latitude, longitude, gfw_integrated_alerts__date, umd_glad_landsat_alerts__confidence, umd_glad_sentinel2_alerts__confidence, wur_radd_alerts__confidence, gfw_integrated_alerts__confidence FROM data WHERE gfw_integrated_alerts__date >= '${beginDate.format('YYYY-MM-DD')}' AND gfw_integrated_alerts__date <= '${endDate.format('YYYY-MM-DD')}' AND umd_tree_cover_density_2010__tree_cover_2022 = true&geostore_origin=rw&geostore_id=423e5dfb0448e692f97b590c61f45f22`,
            json: `${config.get('dataApi.url')}/dataset/gfw_integrated_dist_alerts/latest/download/json?sql=SELECT latitude, longitude, gfw_integrated_alerts__date, umd_glad_landsat_alerts__confidence, umd_glad_sentinel2_alerts__confidence, wur_radd_alerts__confidence, gfw_integrated_alerts__confidence FROM data WHERE gfw_integrated_alerts__date >= '${beginDate.format('YYYY-MM-DD')}' AND gfw_integrated_alerts__date <= '${endDate.format('YYYY-MM-DD')}' AND umd_tree_cover_density_2010__tree_cover_2022 = true&geostore_origin=rw&geostore_id=423e5dfb0448e692f97b590c61f45f22`
        },
