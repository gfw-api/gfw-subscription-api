...
class GladLService {
    ...
    static getURLForAdmin0(startDate, endDate, country) {
        const sql = `SELECT * FROM data WHERE iso='${country}' AND alert__date >= '${startDate}' AND alert__date <= '${endDate}'`;
        return `${DATASET_GLAD_L_ADM_0}?sql=${sql}`;
    }

    static getURLForAdmin1(startDate, endDate, country, region) {
        const sql = `SELECT * FROM data WHERE iso='${country}' AND adm1='${region}' AND alert__date >= '${endDate}' AND alert__date <= '${endDate}'`;
        return `${DATASET_GLAD_L_ADM_1}?sql=${sql}`;
    }

    static getURLForAdmin2(startDate, endDate, country, region, subregion) {
        const sql = `SELECT * FROM data WHERE iso='${country}' AND adm1='${region}' AND adm2='${subregion}' AND alert__date >= '${startDate}' `
            + `AND alert__date <= '${endDate}'`;
        return `${DATASET_GLAD_L_ADM_2}?sql=${sql}`;
    }

    static getURLForWDPA(startDate, endDate, wdpa) {
        const sql = `SELECT * FROM data WHERE wdpa_protected_area__id='${wdpa}' AND alert__date >= '${startDate}' AND alert__date <= '${endDate}'`;
        return `${DATASET_GLAD_L_WDPA}?sql=${sql}`;
    }

    static getURLForGeostore(startDate, endDate, geostoreId) {
        const sql = `SELECT * FROM data WHERE geostore__id='${geostoreId}' AND alert__date >= '${startDate}' AND alert__date <= '${endDate}'`;
