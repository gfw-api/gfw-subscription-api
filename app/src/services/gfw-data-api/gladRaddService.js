...
class GladRaddService {
    ...
    static getURLForAdmin0(startDate, endDate, country) {
        const sql = `SELECT * FROM data WHERE iso='${country}' AND wur_radd_alerts__date >= '${startDate}' AND wur_radd_alerts__date <= '${endDate}'`;
        return `${DATASET_GLAD_RADD_ADM_0}?sql=${sql}`;
    }

    static getURLForAdmin1(startDate, endDate, country, region) {
        const sql = `SELECT * FROM data WHERE iso='${country}' AND adm1='${region}' AND wur_radd_alerts__date >= '${startDate}' `
    ...
    static getURLForAdmin2(startDate, endDate, country, region, subregion) {
        const sql = `SELECT * FROM data WHERE iso='${country}' AND adm1='${region}' AND adm2='${subregion}' AND wur_radd_alerts__date >= '${startDate}' `
            + `AND wur_radd_alerts__date <= '${endDate}'`;
        return `${DATASET_GLAD_RADD_ADM_2}?sql=${sql}`;
    }

    static getURLForWDPA(startDate, endDate, wdpa) {
        const sql = `SELECT * FROM data WHERE wdpa_protected_area__id='${wdpa}' AND wur_radd_alerts__date >= '${startDate}' `
            + `AND wur_radd_alerts__date <= '${endDate}'`;
        return `${DATASET_GLAD_RADD_WDPA}?sql=${sql}`;
    }

    static getURLForGeostore(startDate, endDate, geostoreId) {
        const sql = `SELECT * FROM data WHERE geostore__id='${geostoreId}' AND wur_radd_alerts__date >= '${startDate}' `
            + `AND wur_radd_alerts__date <= '${endDate}'`;
